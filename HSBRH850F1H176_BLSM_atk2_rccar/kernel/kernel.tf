...
$  の責任を負わない．
$
$  $Id: kernel.tf 488 2015-12-17 10:49:10Z nces-okajima $
$
	...
	$IF EQ(TSK.STK[tskid], "NULL")$
		...
		$ELSE$
			$core_idx = OSAP.CORE[TSK.OSAPID[tskid]] * TNUM_TPRI$
 			...
 			$TSK.TINIB_STK[tskid] = CONCAT("_kernel_shared_stack_", TSK.SHARED_STK_ID[tskid])$
...
$	// TCBの出力
	...
	$END$$NL$
$	// TCBのテーブル
	TCB * const p_tcb_table[TNUM_TASK] ={$NL$
	$JOINEACH tskid TSK.ID_LIST ",\n"$
...
$IF LENGTH(CNT.ID_LIST)$
	...
	$JOINEACH cntid CNT.ID_LIST ",\n"$
		...
		&(osapinib_table[$CNT.OSAPID[cntid]$]) }
	$END$
	$NL$
	};$NL$
	$NL$
...
$			// アラームコールバックと自動起動属性以外の属性を管理しない
		...
		$END$
		&(osapinib_table[$ALM.OSAPID[almid]$])$SPC$}
	$END$
	$NL$
	};$NL$
	$NL$
...
$IF LENGTH(SCHTBL.ID_LIST)$
	...
	$JOINEACH schtblid SCHTBL.ID_LIST ",\n"$
		...
		&(osapinib_table[$SCHTBL.OSAPID[schtblid]$]) }
	$END$
	$NL$
	};$NL$
	$NL$
...
$	// C2ISR用の出力
	...
	$JOINEACH isrid ISR2.ID_LIST ",\n"$
		...
		&(osapinib_table[$ISR.OSAPID[isrid]$])$NL$
		$TAB$}
	$END$
...
$	// ICISR用の出力
	$JOINEACH iciid ICI.ID_LIST ",\n"$
		$TAB${$NL$
		$TAB$$TAB$NULL,$SPC$
		&(osapinib_table[$ICI.OSAPID[iciid]$])$NL$
		$TAB$}
	$END$
	$NL$
	};$NL$
...
$ 		};$NL$
$ 	$ELSE$
$ 		TOPPERS_EMPTY_LABEL(const MemorySizeType, ici_remain_stksz);$NL$
$ 	$END$
$ 	$NL$
$ $END$
$ 
